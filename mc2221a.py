import os, threading, time, datetime, screen_brightness_control
...
def connect_device():
    try:
        ...
        print('Re-trying to connect to MCP2221A in 5 seconds..', e)
        time.sleep(5)
        connect_device()
...
def connect_sensor():
    try:
        ...
        print('Re-trying to connect to TSL2591 in 5 seconds..', e)
        time.sleep(5)
        connect_device()
    except Exception as e:
        print('Something else happened during connect_sensor..', e)

def get_sensor_values():
    board_i2c = connect_device()
    #print(board_i2c, dir(board_i2c))
    ...
    try:
        ...
        print('get_sensor_values exception..', e)
...
def generate_value_array(delay, run_event, sensor):
    while run_event.is_set():
        try:
            ...
            print('device might have been disconnected, trying again in 5 seconds..', e)
            time.sleep(5)
            get_sensor_values()
            generate_value_array(delay, run_event, sensor)
...
def print_initial_status(args):
    if(args[0] and args[1] > 0):
        print(CSI + "33;40m" + datetime.datetime.today().strftime("[%I:%M%p]") + CSI + "0m Current brightness is set to {0}% and current lux is {1}.".format(args[0], int(round(float(args[1])))))
    else:
